...
#     encodings = tokenizer.batch_encode_plus([question, answer], max_length=max_qa_seq_length, add_special_tokens=True, padding = padding)
#     token_ids, attention_mask = encodings["input_ids"], encodings["attention_mask"]
#     print("shape diff:", token_ids, indexed_tokens)
#     print("mask diff:", input_mask, attention_mask)
...
class SocialIQ_QA_Relevance_Classification(Dataset):
    ...
    def __getitem__(self, index):
        vid_name = self.data.vid_name[index]
        qid = self.data.qid[index]
        question = self.data.q[index]
        a = self.data.a[index]
        label = self.data.relevance[index]
        X_a, _ = prepare_features(question, a)
        return index, X_a, label, question, a, vid_name, qid
